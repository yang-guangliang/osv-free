...
bool RenderFrameHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameHostImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(FrameHostMsg_SmartClipDataExtracted,
          ...
          /* BEGIN OSV_FREE */
    IPC_MESSAGE_HANDLER(FrameHostMsg_MessageFromMainFrameReceived,
                        OnMessageFromMainFrameReceived)
          /* END OSV_FREE */
...
/* BEGIN OSV_FREE */
void RenderFrameHostImpl::ReceiveMessageFromMainFrame(ReceiveMessageFromMainFrameCallback callback) {
        logging::LogMessage("OSV", 2294, logging::LOG_INFO).stream() << "render_frame_host_impl.cc@ReceiveMessageFromMainFrame@1";
        ReceiveMessageFromMainFrameCallback_ = callback;
        Send(new FrameMsg_ReceiveMessageFromMainFrame(routing_id_));
}

void RenderFrameHostImpl::OnMessageFromMainFrameReceived(base::string16 scriptUrl, base::string16 data, bool isOnmessageInvolved) {
        std::string frameOrigin = frame_tree_node_->frame_tree()->GetMainFrame()->last_committed_url().GetOrigin().spec();
        GURL scriptGURL(scriptUrl);
        std::string scriptOrigin = scriptGURL.GetOrigin().spec();
        base::string16 fo16(frameOrigin.begin(), frameOrigin.end());
        base::string16 so16(scriptOrigin.begin(), scriptOrigin.end());
        ReceiveMessageFromMainFrameCallback_.Run(fo16, so16, data, isOnmessageInvolved);
}
/* END OSV_FREE */

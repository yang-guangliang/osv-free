...
/* BEGIN OSV_FREE */
void ReceiveMessageFromMainFrameCallback(
    const ScopedJavaGlobalRef<jobject>& callback,
    const base::string16& frameOrigin,
    const base::string16& scriptOrigin,
    const base::string16& data,
    const bool isOnmessageInvolved) {
  JNIEnv* env = base::android::AttachCurrentThread();
  ScopedJavaLocalRef<jstring> jframeOrigin = ConvertUTF16ToJavaString(env, frameOrigin);
  ScopedJavaLocalRef<jstring> jscriptOrigin = ConvertUTF16ToJavaString(env, scriptOrigin);

  ScopedJavaLocalRef<jstring> jdata = ConvertUTF16ToJavaString(env, data);
  Java_WebContentsImpl_onMessageFromMainFrameReceived(env, jframeOrigin, jscriptOrigin, jdata, isOnmessageInvolved, callback);
...
/* BEGIN OSV_FREE */
void WebContentsAndroid::ReceiveMessageFromMainFrame(
    JNIEnv* env,
    const JavaParamRef<jobject>& obj,
    const JavaParamRef<jobject>& callback) {
        ScopedJavaGlobalRef<jobject> j_callback;
        ...
        RenderFrameHostImpl::ReceiveMessageFromMainFrameCallback mycallback =
                base::Bind(&ReceiveMessageFromMainFrameCallback, j_callback);

        logging::LogMessage("OSV", 2294, logging::LOG_INFO).stream() << "web_contents_android.cc@ReceiveMessageFromMainFrame@1";
        web_contents_->GetMainFrame()->ReceiveMessageFromMainFrame(mycallback);
}
/* END OSV_FREE */

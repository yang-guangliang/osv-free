...
bool RenderFrameImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderFrameImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(FrameMsg_ExtractSmartClipData, OnExtractSmartClipData)
          /* BEGIN OSV_FREE */
          IPC_MESSAGE_HANDLER(FrameMsg_ReceiveMessageFromMainFrame, OnReceiveMessageFromMainFrame)
          /* END OSV_FREE */
...
/* BEGIN OSV_FREE */
static int myrouting_id;
static int myst_size = 10;
static const v8::StackTrace::StackTraceOptions myst_options =
        static_cast<v8::StackTrace::StackTraceOptions>(
            v8::StackTrace::kDetailed |
            v8::StackTrace::kExposeFramesAcrossSecurityOrigins);
static void postMessageToNativeFrameCallback(const v8::FunctionCallbackInfo<v8::Value> &args)
{
        if (args.Length() < 1) {
                return;
        }

        bool isOnmessageInvolved = false;
        v8::String::Utf8Value param1(args[0]->ToString());
        std::string s = std::string(*param1);
        std::string data = "data:" + s + "||-";

        v8::Isolate* isolate = blink::MainThreadIsolate();
        v8::HandleScope handle_scope(isolate);
        v8::Local<v8::StackTrace> myst = v8::StackTrace::CurrentStackTrace(
            isolate, myst_size, myst_options);

        logging::LogMessage("OSV", 1, logging::LOG_INFO).stream() << "render_frame_impl.cc@postMessageToNativeFrameCallback1";

        data += std::to_string(myst->GetFrameCount()) + "-||";

        for (int i = 0; i < myst->GetFrameCount(); i++) {
                v8::Local<v8::StackFrame> frame = myst->GetFrame(i);
                v8::String::Utf8Value scriptName(frame->GetScriptNameOrSourceURL());
                v8::String::Utf8Value functionName(frame->GetFunctionName());
                data += std::string("ScriptName:") + std::string(*scriptName) + "@function:" + std::string(*functionName) + "->";

                if (std::string(*functionName) == "onmessage")
                        isOnmessageInvolved = true;
        }
        logging::LogMessage("OSV", 1, logging::LOG_INFO).stream() << "render_frame_impl.cc@postMessageToNativeFrameCallback2";

        base::string16 s16(data.begin(), data.end());
        v8::String::Utf8Value scriptOrigin(myst->GetFrame(0)->GetScriptNameOrSourceURL());
        std::string scriptOriginStr = std::string(*scriptOrigin);
        base::string16 sos16(scriptOriginStr.begin(), scriptOriginStr.end());
        RenderThread::Get()->Send(new FrameHostMsg_MessageFromMainFrameReceived(myrouting_id, sos16, s16, isOnmessageInvolved));
        logging::LogMessage("OSV", 1, logging::LOG_INFO).stream() << "render_frame_impl.cc@postMessageToNativeFrameCallback3";
}

void RenderFrameImpl::OnReceiveMessageFromMainFrame() {
        logging::LogMessage("OSV", 2294, logging::LOG_INFO).stream() << "render_frame_impl.cc@OnReceiveMessageFromMainFrame@0";
        v8::Isolate* isolate = blink::MainThreadIsolate();
        v8::HandleScope handle_scope(isolate);
        v8::Local<v8::Context> context = frame_->MainWorldScriptContext();
        if (context.IsEmpty()) {
                logging::LogMessage("OSV", 2294, logging::LOG_INFO).stream() << "render_frame_impl.cc@OnReceiveMessageFromMainFrame : context is empty.";
                return;
        }
        logging::LogMessage("OSV", 2294, logging::LOG_INFO).stream() << "render_frame_impl.cc@OnReceiveMessageFromMainFrame@1";

        myrouting_id = routing_id_;

        v8::Context::Scope context_scope(context);

        v8::Local<v8::Object> global = context->Global();
        global->Set(v8::String::NewFromUtf8(isolate, "postMessageToNativeFrame"),
                    v8::FunctionTemplate::New(isolate, postMessageToNativeFrameCallback)->GetFunction());
}
/* END OSV_FREE */
